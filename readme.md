...
If you set the concrete position, row will have these position and the other rows will be correctly reordered. If you delete some row, other rows will be correctly reordered.
...
> `NULL` value in `position` column means - move this row to the end - bacause of it - the `position` column must be NULLable

> For bigger tables with groups it is a good thougth to add index for some or all group columns 
...
> - CTE queries - when you're doing more operations (INSERT and UPDATE and DELETE) on one table, some is not catched by the trigger - to be honest - I don't know why, but I'm not expecting these type of queries in real life - _see tests_ 
...
This guarantees unbroken numeric series. The magic is to mix existing rows with new/updated/deleted rows in a correct order and partition it with correct groups.
...
- UPDATE is the tricky one. First, we will perform it only if there is some row with changed position. Then we will take all existing rows from the table, except rows with changed position (we can detect it from referencing tables `table_new` and `table_old`), also set position plus `0.1` and `UNION` this with rows with changed position. We need to do here one more trick - if for some row has the new position greater than the old position - we need to add `0.2` to the row position. Because the row is moved down, we must put if after the existing one

The part with groups is more complex. Because we don't want to recalculate whole table with all groups, we need to get only groups with a "significant change" and update only these. What is significant change?
...
For INSERT or DELETE, the algorithm is similar to the table without groups. We just don't do recalculate for the whole table, but first, we generate condition that will take only groups with a significant change - that's all groups in `table_new` or `table_old`.

UPDATE is the most complex part in the whole algorithm. We need to detect changes in the old groups and in the new groups together and recalculate all these data.
...
Next helpers are for conditions that works with actual data from referencing tables and detects all groups with some significant change for old data (`v_table_old_where_sql`) and new data (`v_table_new_where_sql`).

With all these helper variables we can prepare SELECT that took all existing rows from the table where some group has some significant change in old or new data, except updated data where some position is changed or some group is changed. In these rows is also the trick with plus `0.1` for position. These rows are `UNION` with updated data, where position is changed or some group is changed. There is also the trick with plus `0.2` to position, if new position is greater than old position, but here only for rows, where the group is not changed. It is because when group is changed, you want to put record before the existing one and only for moving in the current group you must add if after.   

> Optimizations: whole trigger tries to be as efficient as it could, so in general
> - no action is taken if no rows was inserted, updated or deleted - execution is stopped as soon as possible
> - no action is taken if no significant change is made for update
> - update are only rows with change position in final recalculation
...
- tests: rewrite test scenarios to separate test cases - every test have defined data, and it's not depending on previous test 